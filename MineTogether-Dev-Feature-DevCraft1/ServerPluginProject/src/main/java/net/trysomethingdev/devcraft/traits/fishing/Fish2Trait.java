package net.trysomethingdev.devcraft.traits.fishing;
...
import net.trysomethingdev.devcraft.traits.others.FollowCustomTrait;
...
public class Fish2Trait extends Trait {
    ...
    public void run() {
        ...
        {
            npc.removeTrait(Fish2Trait.class);
            npc.getOrAddTrait(Fish2Trait.class);
    ...
    private void GetFishingLocation() {
        ...
        Location playerLocation = Bukkit.getPlayer(plugin.getMainPlayerUserName()).getLocation();

            if(npc.getEntity().getLocation().distance(playerLocation) < 5 && !scanning){
                scanning = true;
                new DelayedTask(() -> {
                    this.lookingForLocation = true;
                   Log("StartFishingAboutToBeCalled");
                startFishing();
                }, 20 * 1);
            }
            else{
                Log("Navigating to Player Location");
                npc.getNavigator().setTarget(playerLocation);
            }
    }
    ...
    private void SearchForNearbyFishingLocation(Location search) {
        ...
        for (int x = -20; x < 20; x++) {
            ...
            Location spotToCheck = new Location(npc.getEntity().getWorld(),x,startingPlayerLocation.getY(),i).add(startingPlayerLocation.getX(),0,startingPlayerLocation.getZ())   ;
              ...
              if(fishingLocation.getBlock().getRelative(BlockFace.UP).getType() == Material.AIR)
                {
                    ...
                    if(foundSpot)
                    {
                        var nearbyEntities = npc.getEntity().getWorld().getNearbyEntities(validGroundToFishFrom,20,20,20);
                        double closestEntityDistance = 10;
